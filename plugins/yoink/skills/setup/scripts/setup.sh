...
while [[ $# -gt 0 ]]; do
  case $1 in
    -h|--help)
      cat << 'HELP_EOF'
Yoink - Test curation + dependency decomposition (no loop)

USAGE:
  /yoink [PROMPT...] [OPTIONS]
...
EXAMPLES:
  /yoink Decompose litellm --url https://github.com/BerriAI/litellm
  /yoink --url https://github.com/org/repo --package mypackage Evaluate deps
HELP_EOF
      exit 0
      ;;
    --url)
...
if [[ -z "$PROMPT" ]]; then
  ...
  echo "" >&2
  echo "   Examples:" >&2
  echo "     /yoink Decompose litellm --url https://github.com/BerriAI/litellm" >&2
  echo "     /yoink --url https://github.com/org/repo Evaluate deps" >&2
  echo "" >&2
  echo "   For all options: /yoink --help" >&2
  exit 1
fi

if [[ -z "$REPO_URL" ]]; then
  echo "❌ Error: --url is required for yoink" >&2
  exit 1
fi
...
if [[ ! -d "$YOINK_PKG" ]]; then
  ...
  echo "  → ./$YOINK_PKG/ (already exists, skipped)"
fi

echo ""
echo "━━━ Step 2/3: Cloning repo & copying reference to .yoink/ ━━━"
echo "  URL: $REPO_URL"
