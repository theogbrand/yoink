...
# Decomposition Implementer — Build diy_{sub_package}/

Read `.claude/inner-diy-loop.local.md` for your runtime input variables. Your goal is to build `diy_{sub_package}/` — a from-scratch replacement for `{sub_package}`. Success means the `diy_{top_package}/` test suite passes entirely against your implementation.

## Input

Read from `.claude/inner-diy-loop.local.md` (markdown body, after YAML frontmatter):
...
- **functions_to_replace**: Specific functions/classes used by diy_{top_package}/
...
- **new_imports**: List of external libraries that diy_{sub_package}/ now imports as a result
