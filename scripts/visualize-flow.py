#!/usr/bin/env python3
"""
Generate a deterministic text visualization of how slash commands
orchestrate sub-agents.

Parses command markdown files using these conventions:

  Sections:       ## headers (e.g., "## Phase 0: Test Curation")
  Steps:          ### N. headers (e.g., "### 1. Dequeue"), sequential per section
  Agents:         **agent-name** agent
  Conditionals:   - **Condition** -> action
  Loop start:     **Begin loop.** (standalone bold line before the first loop step)
  Loop end:       **Loop back to step N.** (standalone bold line after the last loop step)

Usage:
  python scripts/visualize-flow.py              # print to stdout
  python scripts/visualize-flow.py --check      # exit 1 if FLOW.md is stale
  python scripts/visualize-flow.py --write      # write to FLOW.md
  python scripts/visualize-flow.py --lint       # check conventions only
"""
...
OUTPUT_FILE = Path(__file__).resolve().parent.parent / "FLOW.md"
...
def generate_flow() -> str:
    ...
    output_lines = [
        "# Orchestration Flow",
        "",
        "Auto-generated by `scripts/visualize-flow.py`. Do not edit manually.",
        "",
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Visualize command orchestration flow")
    parser.add_argument("--write", action="store_true", help="Write output to FLOW.md")
    parser.add_argument(
        "--check", action="store_true", help="Check if FLOW.md is up to date"
    )
    parser.add_argument(
        "--lint", action="store_true", help="Check command files follow conventions"
    )
    ...
    if args.check:
        if not OUTPUT_FILE.exists():
            print(
                ...
                "Run: uv run python scripts/visualize-flow.py --write"
            )
            sys.exit(1)
